...
def __rand_int(a: int, b: int, reverse_density: bool = False, density_enforce: int = 2) -> int:
    ...
    return floor((b - a + 1) * s) + a


def sample_uniform_box(width: int, height: int, min_width: int = 1, min_height: int = 1) -> Box:
...
def sample_wide_box(width: int, height: int, min_width: int = 1, min_height: int = 1, density_enforce: int = 2) -> Box:
